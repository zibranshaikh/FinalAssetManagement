...
<td><%=cc.getEid1()%></td>
<td><%=cc.getName()%>${a1.name}</td>
<td><%=cc.getAddress()%>${a1.address}</td>
<td><%=cc.getMobile()%>${a1.mobile}</td>
<td><%=cc.getEmail()%>${a1.email}</td>
<td><%=cc.getDesignation()%>${a1.designation}</td>
<td><%=cc.getMid1()%>${a1.mid1}</td>
<td><%=cc.getDateofjoin()%>${a1.dateofjoin}</td>
  <%  int st=0;
      String status;
      st=cc.getStatus(); 
      if(st==1)
...
<td><input type="hidden" value="${user}" name="mid" /></td>
</tr>
<%} %>
</table>
</pre>
